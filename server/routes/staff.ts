  ...
  wrap((req) => {
    ...
    return {
      ...
      stats: {
        ...
        avgComprehension: avg(cards.map((c) => c.comprehension)),
        avgMastery: avg(cards.map((c) => c.mastery)),
...
staffRouter.get(
  ...
  wrap((req) => {
    ...
    const feedback = {
      ...
      comments: fb
        .filter((f) => f.comment?.trim() && (f.kind !== 'course' || f.courseId === course.id))
        .sort((a, b) => b.createdAt.localeCompare(a.createdAt))
        .slice(0, 5)
        .map((f) => ({ id: f.id, name: d.users.find((u) => u.id === f.userId)?.name ?? 'Learner', comment: f.comment, stars: f.stars ?? null, about: f.kind === 'platform' ? 'The LMS' : 'This course', at: f.createdAt })),
    };
