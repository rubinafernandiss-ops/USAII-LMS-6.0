...
interface MetricsData {
  ...
  rubrics: {
    ...
    criteria: { id: string; label: string; met: number; partly: number; notMet: number; percent: number | null }[];
  }[];
            ...
            {data.rubrics.map((r) => (
              <Card key={r.lessonId}>
                ...
                <div className="mt-3 space-y-2.5">
                  {r.criteria.map((c) => {
                    const n = c.met + c.partly + c.notMet;
                    return (
                      <div key={c.id} className="grid items-center gap-2 sm:grid-cols-[1fr_240px_60px]">
                        <span className="text-sm">{c.label}</span>
                        <div className="flex h-2.5 overflow-hidden rounded-full bg-mist" title={`Met ${c.met} · Partly met ${c.partly} · Not met ${c.notMet}`}>
                          {n > 0 && (
                            <>
                              <div style={{ width: `${(c.met / n) * 100}%`, background: '#00C77F' }} />
                              <div style={{ width: `${(c.partly / n) * 100}%`, background: '#F5A300' }} />
                ...
                <div className="mt-3 flex gap-4 text-xs text-ink-faint">
                  ...
                  <span><span className="mr-1 inline-block h-2 w-2 rounded-full bg-[#F5A300]" />Partly met</span>
