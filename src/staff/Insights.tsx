...
interface ActivityData {
  ...
  resources?: { guideViews: number; guideViewers: number; guideDownloads: number; workDownloads: number; workDownloaders: number };
  feedback?: {
    recommendYes: number;
...
function ResourcesCard({ r, total }: { r: NonNullable<ActivityData['resources']>; total: number }) {
  const tiles = [
    ...
    { label: 'Study Guide PDF downloads', value: String(r.guideDownloads), sub: 'From the Learning page', color: '#8B3DFF' },
    { label: 'Downloaded their own work', value: `${r.workDownloaders} of ${total}`, sub: `${r.workDownloads} download${r.workDownloads === 1 ? '' : 's'} from Your Journey`, color: '#00C77F' },
  ];
  return (
    <Card>
