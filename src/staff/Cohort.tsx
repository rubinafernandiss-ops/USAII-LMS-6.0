...
interface CohortData {
  ...
  cards: CohortCard[];
  stats: null | { total: number; needsSupport: number; avgConfidence: number | null; avgCheck: number | null; avgCompletion: number | null; avgEngagement: number | null; avgComprehension: number | null; avgMastery: number | null; credentials: number; pendingQuestions: number };
}
      ...
      {!s ? (
        ...
        <>
          <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">
            ...
            <Tile label="Average Comprehension" value={pct(s.avgComprehension)} color={scoreColor(s.avgComprehension)} />
            <Tile label="Average Mastery" value={pct(s.avgMastery)} color={scoreColor(s.avgMastery)} />
                  ...
                  <tbody className="divide-y divide-line">
                    {rows.map((c) => (
                      <tr key={c.user.id} className="transition hover:bg-mist/60">
                        ...
                        <td className="px-4 py-3 font-bold" style={{ color: c.comprehension === null ? '#8A8AA3' : scoreColor(c.comprehension) }}>
                          {c.comprehension === null ? <span className="font-normal">Not measured yet</span> : `${c.comprehension}%`}
                        </td>
                        <td className="px-4 py-3 font-bold" style={{ color: c.mastery === null ? '#8A8AA3' : scoreColor(c.mastery) }}>
                          {c.mastery === null ? <span className="font-normal">Building evidence</span> : `${c.mastery}%`}
                        </td>
                        <td className="px-4 py-3">
                          {c.credential ? (
