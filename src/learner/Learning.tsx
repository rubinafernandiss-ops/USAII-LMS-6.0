...
import { Award, BookOpen, CheckCircle2, ChevronDown, Circle, Clock, Download, Eye, FileCheck2, FileText, Film, Headphones, Link2, Lock, PlayCircle } from 'lucide-react';
...
function Overview({ item }: { item: CourseItem }) {
  ...
  const c = item.course;
  const download = () => {
    downloadStudyGuidePdf(c);
    if (!readOnly) track(c.id, 'guide_download');
  };
  const g = c.grading;
      ...
      <div className="mt-4 space-y-2">
        ...
        <button
          onClick={() => {
            ...
            track(course.id, 'guide_download');
          }}
