...
import { InfoTip } from './LearningMetrics';
...
export function DoingCard({ item }: { item: CourseItem }) {
  ...
  const mt = s.metrics;
  // Completion stays separate from the two learning metrics: finishing lessons is not the same as understanding them.
  const rings: { label: string; value: number | null; color: string; pending?: string; info?: string }[] = [
    { label: 'Course completed', value: s.completion.percent, color: '#8B3DFF' },
    { label: METRIC_TEXT.comprehension.name, value: mt.comprehension, color: scoreColor(mt.comprehension), pending: METRIC_TEXT.comprehension.pending, info: METRIC_TEXT.comprehension.info },
    { label: METRIC_TEXT.mastery.name, value: mt.mastery, color: scoreColor(mt.mastery), pending: METRIC_TEXT.mastery.pending, info: METRIC_TEXT.mastery.info },
  ];
  return (
    <Card>
        ...
        {rings.map((r) => (
          <div key={r.label} className="flex flex-col items-center text-center">
            <Ring
              ...
              stroke={narrow ? 8 : 10}
              color={r.color}
              label={r.value === null && r.pending ? <span className="block px-2 text-[11px] font-semibold leading-tight text-ink-faint sm:text-xs">{r.pending}</span> : undefined}
            />
            <div className="mt-2 flex items-center gap-0.5 text-[13px] font-semibold leading-tight sm:text-sm">
              {r.label}
              {r.info && <InfoTip label={r.label} text={r.info} align={r.label === METRIC_TEXT.mastery.name ? 'right' : 'left'} />}
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-2xl bg-mist px-4 py-3">
          <div className="text-xs text-ink-faint">Chance of passing</div>
          <div className="font-display text-xl font-bold" style={{ color: p.evidence === 'none' ? undefined : scoreColor(p.passConfidence) }}>
            {p.evidence === 'none' ? 'After your first quiz' : `${p.passConfidence}%`}
          </div>
        </div>
        <div className="rounded-2xl bg-mist px-4 py-3">
...
/**
 * Comprehension for each learning objective (one per lesson), from the same numbers as the
 * course-level metric. Nothing is shown until the course-level Comprehension is measured.
 */
export function TopicsCard({ item, readOnly }: { item: CourseItem; readOnly?: boolean }) {
  const m = item.snapshot.metrics;
  const byId = new Map(m.objectives.map((o) => [o.objectiveId, o]));
  const due = new Set(item.snapshot.topics.filter((t) => t.reviewDue).map((t) => t.lessonId));
  const measured = m.comprehension !== null;
  return (
    <Card>
      <div className="flex items-center gap-1">
        <h3 className="text-lg font-bold">Comprehension by topic</h3>
        <InfoTip label="Comprehension by topic" text="Each lesson teaches one topic. This is your Comprehension on each topic, from its knowledge check and any final assessment questions about it. Your overall Comprehension is the average of these topics." />
      </div>
      {!measured && <p className="mt-1 text-sm text-ink-soft">Not measured yet. Topics appear after your first two scored checkpoints.</p>}
      <div className="mt-4 space-y-5">
        {item.course.modules.map((mod) => (
          <div key={mod.id}>
            <div className="mb-1.5 text-xs font-semibold text-ink-faint">{mod.title}</div>
            <div className="space-y-1">
              {mod.lessons.map((l) => {
                const v = measured ? (byId.get(l.id)?.comprehension ?? null) : null;
                return (
                  <button
                    key={l.id}
                    type="button"
                    disabled={readOnly}
                    onClick={() => navigate(`study/${item.course.id}/${l.id}`)}
                    className="grid w-full grid-cols-[1fr_120px_110px] items-center gap-3 rounded-xl px-2 py-1.5 text-left text-sm transition enabled:hover:bg-mist"
                  >
                    <span className="flex min-w-0 items-center gap-2">
                      <span className="truncate">{l.topic || l.title}</span>
                      {due.has(l.id) && (
                        <Pill color="purple">
                    ...
                    <Bar value={v} color={scoreColor(v)} height={7} />
                    <span className="text-right text-xs font-semibold" style={{ color: v === null ? '#8A8AA3' : scoreColor(v) }}>
                      {v === null ? (l.check.length ? 'Not measured yet' : 'No check') : `${v}%`}
                    </span>
                  </button>
                );
              })}
